...
page=page.replace('120 integer sequences','140 integer sequences').replace('120 sequences','140 sequences').replace('120 sequence previews','140 sequence previews').replace('"numberOfItems": 120','"numberOfItems": 140').replace('24,000,000 computed points','28,000,000 computed points')
# Preserve the existing social artwork and its image metadata.
p.write_text(page)
p=root/'explore.html'
s=p.read_text().replace('120 integer sequences','140 integer sequences').replace('>120 sequences<','>140 sequences<');p.write_text(s)
print('140 gallery cards; 20 new previews; existing design and controls preserved.')
